...
class ProductoController extends Controller
{
    ...
    public function create()
    {
        ...
        return view('productos.form', [
            ...
            'otrosProductos' => Producto::activo()->orderBy('nombre')->get(['id','nombre']),
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $this->syncExtras($request, $producto);
    ...
    {
        $this->requireAdmin();
        $producto = Producto::with(['categoria','subcategoria','proveedor','comboItems.componente','presentaciones'])->findOrFail($id);
        ...
        return view('productos.form', [
            ...
            'proveedores'    => Proveedor::orderBy('nombre')->get(),
            'otrosProductos' => Producto::activo()->where('id','!=',$id)->orderBy('nombre')->get(['id','nombre']),
        ]);
    }
    ...
    public function search(Request $request)
    {
        $productos = Producto::activo()->search($request->get('q',''))->limit(15)->get([
            'id','nombre','sku','precio_venta','precio_mayoreo','precio_compra','stock','exento'
        ]);
        $productos->each(fn($p) => $p->append('precio_con_iva'));
    ...
    private function validatedData(Request $request, ?int $ignoreId = null): array
    {
        ...
        return $request->validate([
            ...
            'tipo_comision'   => 'nullable|in:ninguno,porcentaje,monto_fijo',
            'valor_comision'  => 'nullable|numeric|min:0',
        ]);
    }

    /** Guarda/reemplaza los componentes de un combo y las presentaciones extra de un producto. */
    private function syncExtras(Request $request, Producto $producto): void
    {
        if ($request->filled('combo_items')) {
            $items = json_decode($request->input('combo_items'), true) ?: [];
            $producto->comboItems()->delete();
            foreach ($items as $it) {
                if (empty($it['componente_id']) || (int)$it['componente_id'] === $producto->id) continue;
                \App\Models\ProductoComboItem::create([
                    'producto_id'   => $producto->id,
        ...
        if ($request->filled('presentaciones')) {
            $pres = json_decode($request->input('presentaciones'), true) ?: [];
            $producto->presentaciones()->delete();
            foreach ($pres as $p) {
                if (empty($p['nombre']) || !isset($p['precio'])) continue;
                \App\Models\ProductoPresentacion::create([
                    'producto_id' => $producto->id,
                    'nombre'      => $p['nombre'],
                    'factor'      => max(0.01, (float)($p['factor'] ?? 1)),
