...
<div class="card">
  <div class="card-body p-0">
    ...
    <table class="table" id="prodTable">
      <thead><tr><th>SKU</th><th>Nombre</th><th>Categoría</th><th>P. Compra</th><th>P. Sin IVA</th><th>P. Con IVA</th><th>Stock</th><th>Acciones</th></tr></thead>
      <tbody>
        ...
        <tr>
          ...
          <td>
            ...
            @endif
            {{ $p->nombre }}
            @if($p->exento)<span class="badge badge-info" style="font-size:.6rem">EXENTO</span>@endif
            @if($p->es_combo)<span class="badge badge-warning" style="font-size:.6rem">COMBO</span>@endif
          </td>
          <td>{{ $p->categoria->nombre ?? '—' }}</td>
          <td>${{ number_format($p->precio_compra,2) }}</td>
          <td>${{ number_format($p->precio_venta,2) }}</td>
          <td><strong>${{ number_format($p->precio_con_iva,2) }}</strong></td>
          <td>
        ...
        @empty
        <tr><td colspan="8" class="text-center text-muted" style="padding:2rem">Sin productos.</td></tr>
        @endforelse
      </tbody>
    </table>
