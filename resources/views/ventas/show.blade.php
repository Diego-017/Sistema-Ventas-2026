...
<div class="card receipt">
  ...
  <div class="receipt-footer">
    <div class="total-row"><span>Gravado:</span><span>${{ number_format($venta->subtotal - $venta->venta_exenta,2) }}</span></div>
    @if($venta->venta_exenta > 0)
    <div class="total-row"><span>Venta Exenta:</span><span>${{ number_format($venta->venta_exenta,2) }}</span></div>
    @endif
    <div class="total-row"><span>IVA (13%):</span><span>${{ number_format($venta->impuesto,2) }}</span></div>
    @if($venta->percepcion > 0)
    <div class="total-row"><span>Percepción (1%):</span><span>${{ number_format($venta->percepcion,2) }}</span></div>
    @endif
    @if($venta->retencion > 0)
    <div class="total-row" style="color:#e74a3b"><span>Retención (1%):</span><span>-${{ number_format($venta->retencion,2) }}</span></div>
    @endif
    @if($venta->descuento > 0)
    <div class="total-row" style="color:#e74a3b"><span>Descuento:</span><span>-${{ number_format($venta->descuento,2) }}</span></div>
    @endif
    <div class="total-row total-final"><span>TOTAL A PAGAR:</span><span>${{ number_format($venta->total,2) }}</span></div>
  </div>
